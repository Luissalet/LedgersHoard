...
export function MonthPicker({ value, onChange }) {
  return (
    <div className="inline-flex items-center gap-1">
      <button type="button" className="btn btn-sm" aria-label="Mes anterior" onClick={() => onChange(addMonths(value, -1))}>‹</button>
      <input
        type="month"
      ...
      />
      <button type="button" className="btn btn-sm" aria-label="Mes siguiente" onClick={() => onChange(addMonths(value, 1))}>›</button>
    </div>
  );
}
